...
class AIPlayer:
    def __init__(self, player_number: int, timer, max_depth=2, logging_mode="file"):
        self.player_number = player_number
        self.type = 'ai13'
        self.player_string = f'Player {player_number}: ai13'
        self.timer = timer
    ...
    def get_move(self, state: np.array) -> Tuple[int, int]:
        """
        Decide the best move using Iterative Deepening DFS.
        """
        ...
        for depth in range(1, self.max_depth + 1):
            for action in valid_actions:
                score = self.depth_limited_search(state, action, depth)
                if score > best_score:
                    best_score = score
                    best_action = action
    ...
    def depth_limited_search(self, state, action, limit):
        """
        Perform depth-limited search (DLS) from the given action up to the specified depth limit.
        """
        return self.dfs(state, action, self.player_number, limit)

    def dfs(self, state, action, player, depth=3):
        """
        Depth-First Search with a depth limit.
        """
        if depth == 0:
            return self.evaluate_state(state, action, player)

        # Temporarily apply the move
        state[int(action[0]), int(action[1])] = player

        valid_actions = get_valid_actions(state)
        opponent = self.opponent_number if player == self.player_number else self.player_number

        best_score = float('-inf') if player == self.player_number else float('inf')

        for next_action in valid_actions:
            if player == self.player_number:
                score = self.dfs(state, next_action, opponent, depth - 1)
    ...
    def evaluate_state(self, state, action, player):
        """
        Evaluate the state based on the action taken.
        """
        if player == self.player_number:
            return self.evaluate_move(state, action)
        else:
            return -self.evaluate_move(state, action)

    def evaluate_move(self, state, action):
        """
        Evaluate the move by combining heuristic scores.
        """
    ...
    def is_near_fork(self, state, action):
        ...
        # Check for near-complete forks with boundary checks
        if (x > 1 and state[x - 1, y] == self.player_number and state[x - 2, y] == self.player_number and y > 0 and state[x, y - 1] == self.player_number) or \
           (x < dim - 2 and state[x + 1, y] == self.player_number and state[x + 2, y] == self.player_number and y > 0 and state[x, y - 1] == self.player_number) or \
           (x > 1 and state[x - 1, y] == self.player_number and state[x - 2, y] == self.player_number and y < dim - 1 and state[x, y + 1] == self.player_number) or \
           (x < dim - 2 and state[x + 1, y] == self.player_number and state[x + 2, y] == self.player_number and y < dim - 1 and state[x, y + 1] == self.player_number) or \
           (y > 1 and state[x, y - 1] == self.player_number and state[x, y - 2] == self.player_number and x > 0 and state[x - 1, y] == self.player_number) or \
           (y > 1 and state[x, y - 1] == self.player_number and state[x, y - 2] == self.player_number and x < dim - 1 and state[x + 1, y] == self.player_number) or \
           (y < dim - 2 and state[x, y + 1] == self.player_number and state[x, y + 2] == self.player_number and x > 0 and state[x - 1, y] == self.player_number) or \
           (y < dim - 2 and state[x, y + 1] == self.player_number and state[x, y + 2] == self.player_number and x < dim - 1 and state[x + 1, y] == self.player_number):
            return True
    ...
    def get_chain_length(self, state, action):
        visited = set()
        return self._dfs_chain(state, action, visited)
    ...
    def assign_heuristic_score(self, state, action):
        ...
        control_score = self.evaluate_territorial_control(action)

        score += pattern_score * 2  # Increase offensive weights
        score += chain_length * 3
        score += control_score * 2
    ...
    def _dfs_chain(self, state, action, visited):
        """
        A depth-first search to calculate the length of the chain starting from the given action.
        """
        if action in visited:
            return 0

        visited.add(action)
        chain_length = 1  # Counting the current piece

        neighbors = get_neighbours(self.board_dim, action)
        for neighbor in neighbors:
            if state[neighbor[0], neighbor[1]] == self.player_number:
                chain_length += self._dfs_chain(state, neighbor, visited)
    ...
    def get_opening_moves(self, state):
        ...
        # If corners and adjacent moves are mostly filled, control the center of the board
        if self.is_valid_move(state, center):
            return [center]  # Aggressively control the center early on

        # Or prioritize adjacent to center for offensive control:
        adjacent_to_center = [
            (center[0]-1, center[1]),
            (center[0]+1, center[1]),
            (center[0], center[1]-1),
