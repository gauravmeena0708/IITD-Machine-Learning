#predict_custom_data.py(processing 32ch data for 19ch model)
import os
import numpy as np
import pandas as pd
import mne
...
MODEL_PATH = '/home/scai/mtech/aib242286/scratch/ail721/1804/saved_models/best_model_epoch_54_val_acc_0.7091.keras'
DATA_FILES = [
...
#For handling extra . in physio dataset
...
def load_and_preprocess_custom_data(data_files_info, event_ids, target_sfreq, fmin, fmax, tmin, tmax, baseline, montage_name, model_expected_n_channels, model_channels_list):
    ...
    X = X[..., np.newaxis]
    logging.info(f"Prepared data shape before time adjustment: {X.shape}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(MODEL_PATH):
        logging.error(f"Model file not found at {MODEL_PATH}"); exit()
    try:
        model = tf.keras.models.load_model(MODEL_PATH)
        model.summary(line_length=100)
        input_shape = model.input_shape
        logging.info(f"Model loaded. Expected input shape (Batch, Chans, Time, 1): {input_shape}")
        ...
        logging.error(f"Failed to load model or verify channels: {e}"); exit()

    X_custom, y_custom_true = load_and_preprocess_custom_data(
    ...
    if X_custom is None or y_custom_true is None:
        logging.error("Failed to load or preprocess custom data. Exiting."); exit()

    current_n_timesteps = X_custom.shape[2]
    if current_n_timesteps != EXPECTED_N_TIMESTEPS:
        ...
        logging.warning("This might impact performance. Retraining the model with consistent parameters is recommended.")
        if current_n_timesteps > EXPECTED_N_TIMESTEPS:
            logging.info(f"Cropping time dimension from {current_n_timesteps} to {EXPECTED_N_TIMESTEPS}.")
            X_custom = X_custom[:, :, :EXPECTED_N_TIMESTEPS, :]
        else:
            ...
            X_custom = np.pad(X_custom, pad_width, mode='constant', constant_values=0)
        logging.info(f"Adjusted data shape: {X_custom.shape}")


    logging.info(f"Making predictions on {X_custom.shape[0]} trials...")
    try:
        X_custom = X_custom.astype(np.float32)
        predictions_prob = model.predict(X_custom)
        predicted_labels_int = np.argmax(predictions_prob, axis=1)
    except Exception as e:
        logging.error(f"Error during model prediction: {e}"); exit()

    predicted_labels_str = [CLASS_MAP.get(label, f"Unknown:{label}") for label in predicted_labels_int]
    true_labels_str = [CLASS_MAP.get(label, f"Unknown:{label}") for label in y_custom_true]

    print("\n--- Prediction Results ---")
    correct_count = 0
    for i in range(len(predicted_labels_str)):
        is_correct = predicted_labels_str[i] == true_labels_str[i]
        print(f"Trial {i+1}: True = {true_labels_str[i]:<5}, Predicted = {predicted_labels_str[i]:<5} {'(Correct)' if is_correct else '(Incorrect)'}")
        if is_correct: correct_count += 1

    accuracy = correct_count / len(predicted_labels_str) if len(predicted_labels_str) > 0 else 0
    print(f"\nOverall Accuracy on Custom Data: {accuracy:.4f} ({correct_count}/{len(predicted_labels_str)})")
