...
class BernoulliNBClassifier:
    ...
    def calculate_conditional_probs(self, X_train, y_train):
        ...
        for idx, c in enumerate(self.class_order):
            ...
            conditional_probs[idx, :] = (np.sum(X_c, axis=0) + 1) / (X_c.shape[0] + len(self.class_order))
        ...
        for i, sample in enumerate(X_test):  # Loop over test samples
            for j, c in enumerate(self.class_order):
                ...
                for k in range(len(sample)):
                    if sample[k] == 1:
                        likelihood_product *= self.conditional_probs[j, k]
...
if __name__ == "__main__":
    ...
    X_train = model.count_vectorizer_binary(train_texts)
    X_test = model.count_vectorizer_binary(test_texts)
    ...
    print(probas)
    data2 = np.load('./check/bernoulli_probas_test.npy')
    print("given")
    print(data2)
