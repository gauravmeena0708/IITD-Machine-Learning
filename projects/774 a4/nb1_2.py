...
class MultinomialNBClassifier:
    ...
    def count_vectorizer_multinomial(self, corpus, is_train=True):
        ...
        for i, sentence in enumerate(tokenized_sentences):
            unseen_count = 0  # Track the number of unseen words
            for word in sentence:
                if word in self.vocab:
                    j = self.vocab.index(word)
                    count_matrix[i, j] += 1
                else:
                    unseen_count += 1  # Increment unseen word count
            if unseen_count > 0:
                count_matrix[i, unknown_index] = unseen_count  # Set the count for unseen words
        ...
        for i, sample in enumerate(X_test):
            for j, c in enumerate(self.class_order):
                ...
                for k in range(len(sample)):
                    # Multiply by the conditional probability raised to the count of occurrences
...
if __name__ == "__main__":
    ...
    print(probas)
    data2 = np.load('./checker_files_v5/multinomial_probas_test.npy')
    print("given")
    print(data2)
