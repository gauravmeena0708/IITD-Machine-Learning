...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(trainset, batch_size=128, shuffle=False, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Epoch {epoch + 1}/{num_epochs}, Loss: {running_loss/len(train_loader)}, accuracy: {val_acc:.2f}% , time: {int(hours)} hours, {int(minutes)} minutes, {int(seconds)}')
        if epoch+1 >=75:
