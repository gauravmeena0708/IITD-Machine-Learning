...
def make_player(name, num, timer=PLAYER_TIME):
    ...
    return player_classes[name](num, timer)
...
def run_game(game_id, p1, p2, time, dim, blocks, mode):
    ...
    print(game.winner)
    if game.winner == 1:
        return p1, p2, 1, 0  # p1 wins
    elif game.winner == 2:
        return p1, p2, 0, 1  # p2 wins
    else:
        return p1, p2, 0, 0  # Draw or error

def main(n):
    players = [f'ai{i:02d}' for i in range(1, 12)]
    time = 240
    dim = 4
    blocks = 0
    mode = "server"
    ...
    for i in range(n):
        ...
        p1_wins = 0
        p2_wins = 0
        total_games = 3  # Playing 3 games between each pair

        for _ in range(total_games):
            result = run_game(i, p1, p2, time, dim, blocks, mode)
            results.append(result)

    # Process the results
    results_df = pd.DataFrame(results, columns=["P1", "P2", "P1_Score", "P2_Score"])
    summary = results_df.groupby(["P1", "P2"]).sum().reset_index()
    summary["Total_Games"] = total_games
    summary = summary[["Total_Games", "P1", "P2", "P1_Score", "P2_Score"]]

    # Save to Excel
    summary.to_excel("tournament_results.xlsx", index=False)

    print("Tournament results saved to 'tournament_results.xlsx'.")

if __name__ == '__main__':
    n = 50 # Set the number of
    main(n)