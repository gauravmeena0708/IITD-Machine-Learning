...
def main(player1: str, player2: str, time: int, dim: int, mode: str, init_file_name: str = None, blocks: int = 0):
    ...
    game = Game(player1, player2, make_player(player1, 1), make_player(player2, 2), time, board, dim, mode)
    return game

if __name__ == '__main__':
    ai_bots = [f'ai{str(i).zfill(2)}' for i in range(1, 14)]
    for i in range(len(ai_bots)):
        for j in range(i + 1, len(ai_bots)):
            player1, player2 = ai_bots[i], ai_bots[j]
            player_types = ['ai01', 'ai02','ai03', 'ai04','ai05', 'ai06','ai07', 'ai08','ai09', 'ai10','ai11', 'ai12','ai13', 'random', 'human']
            print(player1, player2," ")
            game = main(player1, player2, 240, 6, 'server', None, 0)
            print(game, " ")
"""def run_tournament():
    ai_bots = [f'ai{str(i).zfill(2)}' for i in range(1, 14)]
    results = []
    ...
    for i in range(len(ai_bots)):
        for j in range(i + 1, len(ai_bots)):
            player1, player2 = ai_bots[i], ai_bots[j]
            for match in range(3):  # Each pair plays 3 matches
                board = get_random_board(6, 0)
                game = Game(player1, player2, make_player(player1, 1), make_player(player2, 2), 240, board, 6, "server")
                winner = game.winner
                print(f"{player1} vs {player2} (Match {match + 1}), winner:{winner}")
                results.append({
                    'Match': f'{player1} vs {player2} (Match {match + 1})',
